...
frames_dir = r"J:\Fotos\MyTrailStudio\overlay_frames_final_full_v2"
...
for frame in range(total_frames):
    ...
    if len(route_points) > 1:
        draw.line(route_points, fill=(255,255,255,120), width=4)
    ...
    if frame % 30 == 0 or frame == total_frames - 1:
        ...
        bar_len = 30
        filled = int(bar_len * (frame + 1) / total_frames)
        bar = "#" * filled + "-" * (bar_len - filled)
        sys.stdout.write(f"\r[{bar}] {frame + 1}/{total_frames} ({pct:5.1f}%)")
        sys.stdout.flush()
